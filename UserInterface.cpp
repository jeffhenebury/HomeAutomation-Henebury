...
void UserInterface::showOptionsForTV()
{
	...
	while (true)
	{
		...
		if (deviceInput == 2) {
			...
			cout << "The current channel is set to: " << oldChannel << " degrees.\n";
			cout << "What would you like to change it to?";
			cin >> newChannel;
			...
			if (oldChannel < newChannel) {
				...
				cout << "Great, you've turned the temperature down to: " << myTelevision.getCurrentChannel() << ". Hope something good is on!\n";
			}
		};
		if (deviceInput == 3) {
			//NEED TO FIGURE OUT SCHEDULE
...
void UserInterface:: showOptionsForDevice()
{
	...
	while (true)
	{
		...
		if (deviceInput == 2) {
			cout << "TEST, you got to VIEW SCHEDULE\N";
...
//a one-stop shop for scheduling all devices??
void UserInterface::getDeviceSchedule(Device anyDevice) {
